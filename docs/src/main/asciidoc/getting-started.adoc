...
Since dynamic config doesn't rely on a tc-config.xml file and configured dynamically, therefore the Angela API provides a DynamicConfigManager.dynamicCluster() method to define within the test the different parameters.
It is possible to dynamically attach or detach stripes, which is done with the Tsa.attach() and Tsa.detach() API.
The config tool will activate attached Terracotta servers, which is done by the Tsa.activate() method.
